...
#         print(state.behavior_state)
        ...
        print(state.horizontal_velocity)
        counter = 0
        kit = ServoKit(channels=16)
        for i in range (4):
            for j in range (3):
                print(90+(state.joint_angles[j,i]*180/(1.5*np.pi)))
#                 
        for i in range (4):
            for j in range (3):
# #                 print((state.joint_angles[j,i]+(np.pi/2))*180/(2*np.pi))
                kit.servo[counter].angle = (90+(state.joint_angles[j,i]*180/(1.5*np.pi)))
                counter += 1
        state.ticks += 1
